...
\section*{2D Lumped Rate Model without Pores}
...
\end{alignat}
Here, $t\in (0, T^{\mathrm{end}})$ is the time coordinate, $z\in (0, L)$ is the axial cylinder coordinate, $\rho\in (0, R^{\mathrm{c}})$ is the radial cylinder coordinate, $c^{\mathrm{b}}_i\colon (0, T^\mathrm{end})\times (0, L)\times (0, R^\mathrm{c}) \mapsto \mathbb{R}$ is the bulk liquid concentration, $\varepsilon^{\mathrm{t}}\in (0, 1)$ is the total porosity, $\tilde{u}> 0$ is the apparent interstitial velocity, $\tilde{D}^\mathrm{ax}_i\geq 0$ is the apparent axial dispersion coefficient, and $D^\mathrm{rad}_i\geq 0$ is the radial dispersion coefficient.
...
Here, $c^{\mathrm{s}}_{i}\colon (0, T^\mathrm{end}) \times (0, L)\times (0, R^\mathrm{c}) \mapsto \mathbb{R}$ is the particle solid concentration, $f^\mathrm{bind}_{i}$ is the adsorption isotherm function, $\vec{c}^\mathrm{p}$ is the particle liquid components vector, and $\vec{c}^\mathrm{s}$ is the particle solid components vector.
