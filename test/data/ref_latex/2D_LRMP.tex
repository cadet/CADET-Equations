...
\section*{2D Lumped Rate Model with Pores}
...
In the particles, mass transfer is governed by reaction equations in $(0, T^\mathrm{end}) \times (0, L)\times (0, R^\mathrm{c})$ and for all components
...
Here, $c^{\mathrm{p}}_{i}\colon (0, T^\mathrm{end}) \times (0, L)\times (0, R^\mathrm{c}) \mapsto \mathbb{R}$ is the particle liquid concentration, $c^{\mathrm{s}}_{i}\colon (0, T^\mathrm{end}) \times (0, L)\times (0, R^\mathrm{c}) \mapsto \mathbb{R}$ is the particle solid concentration, $f^\mathrm{bind}_{i}$ is the adsorption isotherm function, $\vec{c}^\mathrm{p}$ is the particle liquid components vector, and $\vec{c}^\mathrm{s}$ is the particle solid components vector.
