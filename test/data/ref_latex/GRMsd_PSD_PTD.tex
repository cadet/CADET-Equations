...
\begin{align}
\frac{\partial c^{\mathrm{p}}_{j,i}}{\partial t} &= \frac{1}{r^2} \frac{\partial }{\partial r} \left( r^2 D_{j,i}^{\mathrm{p}} \frac{\partial c^{\mathrm{p}}_{j,i}}{\partial r} \right) - \frac{1 - \varepsilon^{\mathrm{p}}_{j}}{\varepsilon^{\mathrm{p}}_{j}}f^{\mathrm{bind}}_{j,i}\left( \vec{c}^{\mathrm{p}}, \vec{c}^{\mathrm{s}} \right), \\
\frac{\partial c^{\mathrm{s}}_{j,i}}{\partial t} &=  \frac{1}{r^2} \frac{\partial }{\partial r} \left( r^2 D_{j,i}^{\mathrm{s}} \frac{\partial c^{\mathrm{s}}_{j,i}}{\partial r} \right)  + f^{\mathrm{bind}}_{j,i}\left( \vec{c}^{\mathrm{p}}, \vec{c}^{\mathrm{s}} \right),
\end{align}
...
&= 0.
\end{align}
Here, $r$ is the radial particle coordinate, $c^{\mathrm{p}}_{j,i}\colon (0, T^\mathrm{end}) \times (0, L)\times (0, R^{\mathrm{p}}) \mapsto \mathbb{R}$ is the particle liquid concentration, $c^{\mathrm{s}}_{j,i}\colon (0, T^\mathrm{end}) \times (0, L)\times (0, R^{\mathrm{p}}) \mapsto \mathbb{R}$ is the particle solid concentration, $d_j$ is the particle type volume fraction, $\varepsilon^{\mathrm{p}}_{j}\in (0, 1)$ is the particle porosity, $D^\mathrm{p}_{j,i}> 0$ is the particle diffusion coefficient, $D^\mathrm{s}_{j,i}\geq 0$ is the surface diffusion coefficient, $f^\mathrm{bind}_{j,i}$ is the adsorption isotherm function, $\vec{c}^\mathrm{p}$ is the particle liquid components vector, and $\vec{c}^\mathrm{s}$ is the particle solid components vector.
