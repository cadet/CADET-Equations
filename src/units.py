...
CONVERSION_FACTORS: dict[str, dict[str, float | None]] = {
...
def get_unit(unit_key: str, system: str = "SI") -> str:
    ...
    if system not in UNIT_SYSTEMS:
        raise ValueError(f"Unknown unit system '{system}'. Available systems: {AVAILABLE_SYSTEMS}")
    units = UNIT_SYSTEMS[system]
    if unit_key not in units:
        raise KeyError(f"Unknown unit key '{unit_key}' in system '{system}'. Available keys: {list(units.keys())}")
    return units[unit_key]


def get_conversion_factor(unit_key: str, target: str) -> float | None:
    ...
    if target not in CONVERSION_FACTORS:
        raise ValueError(f"Unknown unit system '{target}'. Available systems: {AVAILABLE_SYSTEMS}")
    factors = CONVERSION_FACTORS[target]
    if unit_key not in factors:
        raise KeyError(f"Unknown unit key '{unit_key}' in system '{target}'.")
    return factors[unit_key]


def format_conversion_factor(factor: float | None) -> str:
