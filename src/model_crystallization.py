...
class Crystallization:
    ...
    def __post_init__(self):
        ...
        self.column_type = st.sidebar.selectbox("Reactor type", ["CSTR", "DPFR"], key=r"cry_column_type")

        if self.column_type == "DPFR":
            self.has_axial_dispersion = (
                st.sidebar.selectbox("Add axial dispersion", ["Yes", "No"], key=r"cry_has_axial_dispersion") == "Yes"
            )

        st.sidebar.write("Crystallization mechanisms")
        self.has_primary_formation = (
            st.sidebar.selectbox(
                "Primary particle formation (nucleation and growth)", ["Yes", "No"], key=r"cry_has_primary_formation"
            )
            == "Yes"
        )
        ...
        if self.has_primary_formation:
            with st.sidebar.expander("Nucleation and growth", expanded=True):
                self.size_dependent_growth = (
                    st.selectbox("Size-dependent growth", ["No", "Yes"], key=r"cry_size_dependent_growth") == "Yes"
                )
                self.has_growth_dispersion = (
                    st.selectbox("Add growth dispersion", ["No", "Yes"], key=r"cry_has_growth_dispersion") == "Yes"
                )
                self.has_secondary_nucleation = (
                    st.selectbox("Add secondary nucleation", ["No", "Yes"], key=r"cry_has_secondary_nucleation")
                    == "Yes"
                )

        self.has_aggregation = st.sidebar.selectbox("Aggregation", ["No", "Yes"], key=r"cry_has_aggregation") == "Yes"
        ...
        if self.has_aggregation:
            with st.sidebar.expander("Aggregation", expanded=True):
                ...
                selected_kernel = st.selectbox("Aggregation kernel", kernel_labels, key=r"cry_aggregation_kernel")
                self.aggregation_kernel_index = kernel_keys[kernel_labels.index(selected_kernel)]

        self.has_fragmentation = (
            st.sidebar.selectbox("Fragmentation", ["No", "Yes"], key=r"cry_has_fragmentation") == "Yes"
        )
    ...
    def model_assumptions(self):
        return {
            "Model assumptions": eq.cry_assumptions(
                self.column_type, self.has_primary_formation, self.has_aggregation, self.has_fragmentation
            )
        }
    ...
    def solver_details(self):
        ...
        if self.available_CADET_Core() >= 0:
            details = [
                (
                    "Size coordinate discretization",
    ...
    def fill_vars_and_params(self):
        ...
        vp = [
            ...
            },
            {
                "Group": 0,
                "Symbol": r"x",
                "Description": "particle size (internal coordinate)",
                "Unit": u("length"),
                "Dependence": r"\text{independent variable}",
            },
            {
                "Group": 1,
                "Symbol": r"n",
                "Description": "particle number density",
                "Unit": u("number_density"),
                "Dependence": r"t, x",
            },
            {
                "Group": 1,
        ...
        if self.column_type == "CSTR":
            vp.append(
                {"Group": 1.5, "Symbol": r"V", "Description": "reactor volume", "Unit": u("volume"), "Dependence": r"t"}
            )
            vp.append(
                {
                    "Group": 2,
            ...
            vp.append(
                {
                    "Group": 3,
                    "Symbol": r"s",
                    "Description": "relative supersaturation",
            ...
            vp.append(
                {
                    ...
                    "Description": "equilibrium solubility",
                    "Unit": u("concentration_mass"),
                    "Dependence": r"\text{constant}",
                }
            )
                ...
                vp.append(
                    {
                        "Group": 4,
                        "Symbol": r"\gamma",
                        "Description": "size-dependence quantifier",
                        "Unit": u("dimensionless"),
                        "Dependence": r"\text{constant}",
                    }
                )
                vp.append(
                    {
                        "Group": 4,
                        "Symbol": r"p",
                        "Description": "size-dependence exponent",
                        "Unit": u("dimensionless"),
                        "Dependence": r"\text{constant}",
                    }
                )
            ...
            vp.append(
                {
                    "Group": 5,
                    "Symbol": r"u",
                    "Description": "primary nucleation exponent",
                    "Unit": u("dimensionless"),
                    "Dependence": r"\text{constant}",
                }
            )
                ...
                vp.append(
                    {
                        "Group": 5,
                        "Symbol": r"b",
                        "Description": "secondary nucleation exponent",
                        "Unit": u("dimensionless"),
                        "Dependence": r"\text{constant}",
                    }
                )
                vp.append(
                    {
                        "Group": 5,
                        "Symbol": r"k",
                        "Description": "suspension density exponent",
                        "Unit": u("dimensionless"),
                        "Dependence": r"\text{constant}",
                    }
                )
            ...
            vp.append(
                {
                    "Group": -1,
                    "Symbol": r"\rho",
                    "Description": "nuclei mass density",
                    "Unit": u("concentration_mass"),
                    "Dependence": r"\text{constant}",
                }
            )
        ...
        if has_frag:
            vp.append(
                {
                    "Group": 8,
                    "Symbol": r"S(x)",
                    "Description": "selection (fragmentation rate) function",
            ...
            vp.append(
                {
                    "Group": 8,
                    "Symbol": r"b(x \mid \lambda)",
                    "Description": "breakage probability density",
                    "Unit": u("inverse_length"),
                    "Dependence": r"x, \lambda",
                }
            )
            vp.append(
            ...
            vp.append(
                {
                    "Group": 8,
                    "Symbol": r"\alpha",
                    "Description": "fragmentation exponent",
                    "Unit": u("dimensionless"),
                    "Dependence": r"\text{constant}",
                }
            )
